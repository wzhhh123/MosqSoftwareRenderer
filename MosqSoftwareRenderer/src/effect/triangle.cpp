...
void Triangle::updateAndRender(std::shared_ptr<RenderTarget> rt) {
		...
		Mosq::getInstance()->fillTriangle(
				Vertex(0, 1, 0, glm::vec4(1,0,0,1)).transform(mat).perspectiveDivide(),
				Vertex(0.5, -0.2, 0, glm::vec4(0, 1, 0, 1)).transform(mat).perspectiveDivide(),
				Vertex(1.2, 0.3, 0, glm::vec4(0, 0, 1, 1)).transform(mat).perspectiveDivide()
				);
}
